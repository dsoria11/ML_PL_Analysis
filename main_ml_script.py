...
GROWTH_DATA_FILE = 'quantum_well_data.xlsx'
...
def load_and_preprocess_data(growth_data_file, growth_data_file_type, extracted_peak_data_file, extracted_peak_file_type):
    ...
    # Check if growth_data_file exists
    if not os.path.exists(growth_data_file):
        print(f"Error: Growth data file {growth_data_file} does not exist.")
        print("Please ensure this file exists and the path/filename in CONFIGURATION is correct")
        return None, None, None

    if growth_data_file_type == 'csv':
        growth_df = pd.read_csv(growth_data_file)
    elif growth_data_file_type == 'excel':
        try:
            ...
            print(f"Error reading Excel file: '{growth_data_file}' {e}")
            return None, None, None
    else:
